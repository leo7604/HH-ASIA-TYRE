...
// Test booking data generator
function createTestData(branchId, branchName) {
  const timestamp = Date.now();
  return {
    customer: {
      full_name: `Test Customer ${branchName}`,
      email: `test.${branchName.toLowerCase().replace(/\s/g, '')}@example.com`,
      phone: '09171234567'
    },
    vehicle: {
      make: 'Toyota',
      model: 'Vios',
      year: 2024,
      plate_number: `TEST-${branchId}-${timestamp.toString().slice(-4)}`
    },
    booking: {
      branch_id: branchId,
      ...
      customer_concern: `Test booking for ${branchName} branch`
    }
  };
}
...
async function testBranchBooking(branch, index) {
  ...
  try {
    ...
    const testData = createTestData(branch.id, branch.area);
    
    const { data: customer, error: customerError } = await supabase
      .from('customers')
      .insert([{
        customer_reference: `CUST-TEST-${Date.now()}-${branch.id}`,
        ...testData.customer,
        total_bookings: 0,
        status: 'ACTIVE'
      }])
